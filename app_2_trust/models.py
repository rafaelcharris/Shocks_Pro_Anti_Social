...
class Group(BaseGroup):
    ...
    def set_payoff_belief(self):
        print("[[ APP_2_TRUST ]] - GROUP/BELIEF PAYOFF - GROUP.MATRIX R2 ==> ", self.subsession.get_group_matrix(), " <== ]]", )
        for p in self.get_players():
            p1 = self.get_player_by_id(2)
            p2 = self.get_player_by_id(1)
        self.subsession.undo_shuffle()
        for p in self.get_players():
            p1 = self.get_player_by_id(2)
            p2 = self.get_player_by_id(1)
        print("[[ APP_2_TRUST ]] - GROUP/BELIEF PAYOFF - GROUP.MATRIX R1 ==> ", self.subsession.get_group_matrix(), " <== ]]",)
        print("[[ APP_2_TRUST ]] - GROUP/BELIEF PAYOFF - ###############################################################")
